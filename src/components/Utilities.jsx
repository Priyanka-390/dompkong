...
const Utilities = () => {
  const cards = myArray.map((e) => (
    <div className="lg:w-1/3 md:w-1/2 w-full px-3 lg:pt-14 md:pt-10 pt-4 overflow-x-clip">
      <div key={myArray.id}
        className="cards relative group hover:bg-[#FDDA60] transition-all duration-300 hover:shadow-[0px_14px_56px_0px_#FDDA6080]
] z-[1] overflow-hidden rounded-[16px] flex flex-col justify-center items-center border border-solid  bg-[#FFFFFF1A] border-[#FFFFFF33] lg:p-6 md:p-5 p-4"
      >
        <span className="text-center" data-aos="fade-right">
          {e.svg}
        </span>
        <p
        ...
        <div className="w-[91px] h-[91px] rounded-full  bg-white blur-[76px] absolute top-0 right-0 z-[-1]"></div>
        <div className="w-[91px] h-[91px] rounded-full bg-white blur-[76px] absolute bottom-0 left-0 z-[-1]"></div>
      </div>
    </div>
  ));
  return (
    <div>
      <div className="container mx-auto max-w-[1140px] px-3">
        <div className="flex justify-center items-center">
          <h2 className="md:text-[64px] md:pb-0 pb-3 text-[40px] font-azosans text-white font-normal leading-[108%]">
            Utilities
          </h2>
        </div>
        <div className="flex flex-row flex-wrap -mx-3 lg:pb-32 md:pb-20 sm:pb-10 pb-8 justify-center">
          {cards}
        </div>
      </div>
    </div>
